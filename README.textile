...
This version only runs test with postgresql. Remove the mysql config from test/config/database.yml.

For a particular Gemfile in test/config/gemfiles, for example Gemfile.rails-4.2.x, run these commands once to get set up:

```
$ bundle install --gemfile=test/config/gemfiles/Gemfile.rails-4.2.x
$ BUNDLE_GEMFILE=test/config/gemfiles/Gemfile.rails-4.2.x DB=postgresql RAILS_ENV=postgresql bundle exec rake db:create
BUNDLE_GEMFILE=test/config/gemfiles/Gemfile.rails-4.2.x DB=postgresql RAILS_ENV=etl_execution bundle exec rake db:create
$ BUNDLE_GEMFILE=test/config/gemfiles/Gemfile.rails-4.2.x DB=postgresql RAILS_ENV=postgresql bundle exec rake db:schema:load
```

Then to run the tests:

```
$ BUNDLE_GEMFILE=test/config/gemfiles/Gemfile.rails-4.2.x DB=postgresql RAILS_ENV=postgresql bundle exec rake test
```
